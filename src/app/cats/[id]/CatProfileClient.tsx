...
import { addCatPhoto, updateCatPhotoCaption, deleteCatPhoto } from "./actions";
...
function CatAvatar({
  ...
  cat: { gradient_from: string; gradient_to: string; name: string };
  size?: "sm" | "lg";
}) {
  const dim = size === "lg" ? "w-40 h-40 md:w-52 md:h-52" : "w-16 h-16";
  return (
    <div
      className={`${dim} rounded-2xl bg-gradient-to-br ${cat.gradient_from} ${cat.gradient_to} flex items-center justify-center flex-shrink-0 relative overflow-hidden`}
    >
...
function PhotoTile({
  ...
  photo: CatPhotoRow;
  isAdmin: boolean;
  onEdit: (photo: CatPhotoRow) => void;
  onDelete: (photo: CatPhotoRow) => void;
}) {
  return (
...
export default function CatProfileClient({
  cat,
  photos,
}: {
  cat: CatRow;
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!addFile) {
      setAddError("Choose a photo first.");
      return;
    }
    setAddBusy(true);
    setAddError("");
    try {
      // Upload from the browser (Server Actions have a small body limit),
      // then let the server action write the row + revalidate.
      const supabase = createClient();
      const ext = addFile.name.split(".").pop()?.toLowerCase() || "jpg";
      const path = `${cat.id}/${crypto.randomUUID()}.${ext}`;
      ...
      const result = await addCatPhoto(cat.id, publicUrl, addCaption);
  ...
  // ── Edit modal ──
  const [editTarget, setEditTarget] = useState<CatPhotoRow | null>(null);
  const [editCaption, setEditCaption] = useState("");
  const [editError, setEditError] = useState("");
  const [editBusy, setEditBusy] = useState(false);

  const openEdit = (photo: CatPhotoRow) => {
    setEditTarget(photo);
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    setEditBusy(true);
    setEditError("");
    const result = await updateCatPhotoCaption(editTarget.id, cat.id, editCaption);
    setEditBusy(false);
    if (result.error) {
  ...
  // ── Delete confirmation ──
  const [deleteTarget, setDeleteTarget] = useState<CatPhotoRow | null>(null);
  ...
  const confirmDelete = async () => {
    ...
    setDeleteBusy(true);
    setDeleteError("");
    const result = await deleteCatPhoto(
      deleteTarget.id,
      cat.id,
  ...
  const guardedEdit = (photo: CatPhotoRow) =>
    requestAccess(() => openEdit(photo));
  const guardedDelete = (photo: CatPhotoRow) =>
    requestAccess(() => { setDeleteTarget(photo); setDeleteError(""); });
    ...
    <div className="max-w-7xl mx-auto px-6 py-12 flex flex-col gap-12 w-full">
      ...
      >
        <form onSubmit={handleAddSubmit} className="flex flex-col gap-4">
          ...
          {addError && (
            <p className="font-mono text-[10px] text-accent-peach">{addError}</p>
          )}

          <button
            type="submit"
            disabled={addBusy}
            className="mt-1 w-full py-2.5 bg-[#E2E2E2] hover:bg-white text-[#131313] font-hanken font-bold text-sm rounded-lg transition-colors cursor-pointer disabled:opacity-60 disabled:cursor-wait"
          >
            {addBusy ? "Uploading…" : "Save to Collection"}
          </button>
        </form>
      </Modal>
      ...
      >
        <form onSubmit={handleEditSubmit} className="flex flex-col gap-4">
          ...
          {editError && (
            <p className="font-mono text-[10px] text-accent-peach">{editError}</p>
          )}
          <button
            type="submit"
            disabled={editBusy}
            className="mt-1 w-full py-2.5 bg-[#E2E2E2] hover:bg-white text-[#131313] font-hanken font-bold text-sm rounded-lg transition-colors cursor-pointer disabled:opacity-60 disabled:cursor-wait"
          >
            {editBusy ? "Saving…" : "Update Caption"}
          </button>
        </form>
      </Modal>
